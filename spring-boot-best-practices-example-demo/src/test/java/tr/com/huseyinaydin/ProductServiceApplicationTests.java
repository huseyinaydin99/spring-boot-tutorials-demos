...
class ProductServiceApplicationTests {
	...
	public void createNewProductTest() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post(ENDPOINT_URL)
				...
				.andExpect(status().isCreated()).andExpect(MockMvcResultMatchers.jsonPath("$.results.id").exists());
	}

	@Test
	public void shouldReturnAllProductsFromDB() throws Exception {
		when(productRepository.findAll())
				.thenReturn(Arrays.asList(new Product(1, "demo1", "desc1", "type1", 1, 1000, "SUP1", "SUP01"),
						new Product(2, "demo2", "desc2", "type2", 2, 2000, "SUP2", "SUP02")));
		mockMvc.perform(MockMvcRequestBuilders.get(ENDPOINT_URL).accept(MediaType.APPLICATION_JSON)).andDo(print())
