...
public class ProductService {
	...
	public ProductResponseDTO createNewProduct(ProductRequestDTO productRequestDTO)
		...
		try {
			...
			log.debug("ProductService:createNewProduct request parameters {}",
					ValueMapper.jsonAsString(productRequestDTO));

			Product productResults = productRepository.save(product);
			productResponseDTO = ValueMapper.convertToDTO(productResults);
			log.debug("ProductService:createNewProduct received response from Database {}",
					ValueMapper.jsonAsString(productRequestDTO));
	...
	public List<ProductResponseDTO> getProducts() throws ProductServiceBusinessException {
		...
		try {
			...
			log.debug("ProductService:getProducts retrieving products from database  {}",
					ValueMapper.jsonAsString(productResponseDTOS));
...
//            }
			...
			log.error("Exception occurred while retrieving product {} from database , Exception message {}", productId,
					ex.getMessage());
	...
	public Map<String, List<ProductResponseDTO>> getProductsByTypes() {
		try {
			...
			log.error(
					"Exception occurred while retrieving product grouping by type from database , Exception message {}",
					ex.getMessage());
